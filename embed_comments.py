...
def main(
    labels_file: str = "../data/flickr30k_images/results.csv",
    output_file: str = "comment_embeddings/comment_embeddings_pc.pt",
):
    """
    ...
    glove_embedding = GloVe(
        name="6B", dim=100, is_include=lambda w: w in set(corpus_vocab)
    )
    ...
    corpus_vocab_count = pd.Series(
        data=corpus_vocab.map(corpus_count).values, index=corpus_vocab.values
    )
    corpus_vocab_prob = corpus_vocab_count / sum(corpus_vocab_count)

    comment_embeddings = sif_embeddings(
        comment_lists, corpus_vocab_count, corpus_vocab_prob, vocab_embeddings
    )
    comment_embeddings_pc = remove_pc(comment_embeddings)
    torch.save(comment_embeddings_pc, output_file)


def preprocess_comments(input_str: str, input_type: str = "dataframe"):
    """
    ...
    if input_type == "dataframe":
        ...
        input_str["comment"] = (
            input_str["comment"].str.lstrip().str.rstrip(" .").str.lower()
        )

        for i in range(len(replace_strings)):
            input_str["comment"] = input_str["comment"].str.replace(
                alter_strings[i], replace_strings[i]
            )
...
def create_flat_comments_series(labels_df: pd.DataFrame):
    """
...
def compute_pc(X: np.array, npc: int = 1):
    """
...
def remove_pc(X: torch.Tensor, npc: int = 1):
    """
...
def sif_embeddings(
    ...
    for comment_number, comment in enumerate(comment_lists):
        word_count = 0
        comment_embedding = torch.zeros(
            [embedding_size], dtype=torch.float16
        )  # Summary vector
        ...
        for word in comment:
            ...
            if word_in_corpus:
                try:
                    word_embedding = vocab_embeddings[
                        corpus_vocab_count.index.get_loc(word)
                    ].clone()
                    comment_embedding = torch.add(
                        comment_embedding,
                        (alpha / (alpha + corpus_vocab_prob[word])) * word_embedding,
                    )
                    word_count += 1
                except:
        ...
        comment_embeddings = torch.cat(
            [comment_embeddings, comment_embedding.reshape([1, embedding_size])], 0
        )
